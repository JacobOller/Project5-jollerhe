...
private:
    ...
    unsigned int collisionCounter; // New variable. Counter for number of collisions.
...
public:
    ...
    optional<Keyable> find(string key) {
        ...
        for (const pair& p : table[index]) {
            // Adds 1 to collision if find method successfully finds an item at the index
            collisionCounter++;
            if (p.key == key) {
                // We found the item
                return p.value;
    ...
    unsigned long getNumCollisions() const {
